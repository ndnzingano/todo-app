// variáveis com o código SVG dos botões de remove e complete
...
//Usuário clicou no botão de adc:
//Se tem algum texto dentro do "item", adc na lista de todos
document.getElementById('add').addEventListener('click', function() {
    //Pega o valor de item
...
function completeItem() {
    ...
    // se o id for "todo" ainda precisa ser feito, senão é uma atividade que já foi feita  
    var target = (id === 'todo') ? document.getElementById('completed'):document.getElementById('todo');

    
    // remove da lista de todos e adc na posição [0] da lista de completed
    parent.removeChild(item);
    target.insertBefore(item, target.childNodes[0]);

}

// Adc um novo item a lista de todos:
function addItemTodo(text) {
    ...
    //Adc evento para remover item (função acima)
    ...
    //Adc evento para completar item (função acima)
    complete.addEventListener('click', completeItem);
    
    //Insere os elementos (filhos) no elemento-pai
    buttons.appendChild(remove);
    buttons.appendChild(complete);
    item.appendChild(buttons);

    // insere o novo item na posição [0] sempre
